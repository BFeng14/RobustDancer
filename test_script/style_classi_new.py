...
if __name__ == "__main__":
    ...
    # set smpl
    smpl = SMPL(model_path="path_to_smpl/models", gender='MALE',
                batch_size=1)

    # get motion features for the results
    result_features = []
    result_files = glob.glob(output_path + "/*.npy")
    ...
    for result_file in tqdm.tqdm(result_files):
        ...
        keypoints3d = recover_motion_to_keypoints(result_motion[:, 40:20 * 20], smpl)

        # keypoints3d_root = keypoints3d[:, 0:1, :]
        # keypoints3d_del_root = keypoints3d - keypoints3d_root
        # keypoints3d_del_root = keypoints3d_del_root[:400]
        keypoints3d = keypoints3d

        result_features.append(np.concatenate([
            extract_feature(keypoints3d, "kinetic"), extract_feature(keypoints3d, "manual")
        ]))

        real_labels.append(stylename2idx[result_file.split("_")[-2][1:3]])

    pred = classifier.predict(result_features)
